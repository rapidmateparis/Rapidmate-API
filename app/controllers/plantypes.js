...
exports.getItems = async (req, res) => {
  try {
    const getUserQuerye = 'select * from plan_type'
...
exports.getItem = async (req, res) => {
  try {
    ...
    const getUserQuerye = "select * from plan_type where id='"+id+"'"
    const data = await runQuery(getUserQuerye)
    let message="Items retrieved successfully";
    if(data.length <=0){
        message="No items found"
...
const updateItem = async (id,req) => {
    const registerQuery = `UPDATE plan_type SET plan_name ='${req.plan_name}',plan_description='${req.plan_description}' WHERE id ='${id}'`;
...
exports.updateItem = async (req, res) => {
  try {
    ...
    const doesNameExists = await utils.nameExists(plan_name,'plan_type','plan_name')
    if (doesNameExists) {
...
const createItem = async (req) => {
    const registerQuery = `INSERT INTO plan_type (plan_name,plan_description) VALUES ('${req.plan_name}','${req.plan_description}')`;
    const registerRes = await runQuery(registerQuery);
    return registerRes;
}
exports.createItem = async (req, res) => {
  try {
    let error = false;
    const doesNameExists =await utils.nameExists(req.body.plan_name,'plan_type','plan_name')
    if (!doesNameExists) {
...
const deleteItem = async (id) => {
    const deleteQuery = `DELETE FROM plan_type WHERE id ='${id}'`;
...
exports.deleteItem = async (req, res) => {
  try {
    const {id} =req.params
    const getId = await utils.isIDGood(id,'id','plan_type')
    if(getId){
