...
exports.getItems = async (req, res) => {
  try {
    const getUserQuerye = 'select * from payment_method'
...
exports.getItem = async (req, res) => {
  try {
    ...
    const getUserQuerye = "select * from payment_method where id='"+id+"'"
    const data = await runQuery(getUserQuerye)
    let message="Items retrieved successfully";
    if(data.length <=0){
        message="No items found"
...
const updateItem = async (id,req) => {
    const registerQuery = `UPDATE payment_method SET name='${req.name}' WHERE id ='${id}'`;
...
exports.updateItem = async (req, res) => {
  try {
    const { id } = req.params;
    const { name } = req.body;
    const doesNameExists = await utils.nameExists(name,'payment_method','name')
    if (doesNameExists) {
...
const createItem = async (req) => {
    const registerQuery = `INSERT INTO payment_method (name) VALUES ('${req.name}')`;
    const registerRes = await runQuery(registerQuery);
    return registerRes;
}
exports.createItem = async (req, res) => {
  try {
    let error = false;
    const doesNameExists =await utils.nameExists(req.body.name,'payment_method','name')
    if (!doesNameExists) {
...
const deleteItem = async (id) => {
    const deleteQuery = `DELETE FROM payment_method WHERE id ='${id}'`;
...
exports.deleteItem = async (req, res) => {
  try {
    const {id} =req.params
    const getId = await utils.isIDGood(id,'id','payment_method')
    if(getId){
