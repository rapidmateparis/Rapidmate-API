...
exports.getItems = async (req, res) => {
  try {
    const getUserQuerye = 'select * from roles'
...
exports.getItem = async (req, res) => {
  try {
    ...
    const getUserQuerye = "select * from roles where role_id='"+id+"'"
    const data = await runQuery(getUserQuerye)
    let message="Items retrieved successfully";
    if(data.length <=0){
        message="No items found"
...
const updateItem = async (id,name) => {
    const registerQuery = `UPDATE roles SET name ='${name}' WHERE role_id ='${id}'`;
...
exports.updateItem = async (req, res) => {
  try {
    const { id } = req.params;
    const { name } = req.body;
    const doesNameExists = await utils.nameExists(name,'roles','name')
    if (doesNameExists) {
...
const createItem = async (name) => {
    const registerQuery = `INSERT INTO roles (name) VALUES ('${name}')`;
    const registerRes = await runQuery(registerQuery);
    return registerRes;
}
exports.createItem = async (req, res) => {
  try {
    let error = false;
    const doesNameExists =await utils.nameExists(req.body.name,'roles','name')
    if (!doesNameExists) {
...
const deleteItem = async (id) => {
    const deleteQuery = `DELETE FROM roles WHERE role_id ='${id}'`;
...
exports.deleteItem = async (req, res) => {
  try {
    const {id} =req.params
    const getId = await utils.isIDGood(id,'role_id','roles')
    if(getId){
