    ...
    if (orderType == "E") {
      const responseEnterpriseData = await Promise.all(
        responseData.map(async (order) => {
          ...
          const slots = await fetch("SELECT * FROM rmt_enterprise_order_slot WHERE enterprise_order_id = ? and delivery_boy_Id = (SELECT id FROM rmt_delivery_boy WHERE ext_id = ?) and date(slot_date)=date(now())", [order.id, id]);
          return {
            ...order,
            slots,
            locations,
