...
exports.getJoinRequest = async (req, res) => {
  try {
    ...
    SELECT u.ext_id, u.first_name, u.last_name, u.email, u.phone, u.profile_pic, u.role, 
       u.work_type_id, u.company_name, u.industry_type_id, u.is_active, 
       CASE 
...
exports.viewJoinRequest = async (req, res) => {
  try {
    ...
    if (userData[0].roleName == DELIVERY_BOY) {
      queryReq = `SELECT u.ext_id as deliveryboyId,u.first_name,u.last_name,u.email,u.phone,u.profile_pic,u.role,u.work_type_id,u.industry_type_id,u.is_active,e.created_on,c.country_name as country,s.state_name as state,ct.city_name as city,w.work_type,CASE WHEN u.is_active = 1 THEN 'Active' WHEN u.is_active = 2 THEN 'Rejected' WHEN u.is_active = 0 THEN 'Pending' ELSE 'Pending' END AS status FROM vw_rmt_user u LEFT JOIN rmt_delivery_boy e ON u.ext_id = e.ext_id  LEFT JOIN rmt_country c ON e.country_id=c.id LEFT JOIN rmt_state s ON e.state_id=s.id LEFT JOIN rmt_city ct ON e.city_id=ct.id LEFT JOIN rmt_work_type w ON u.work_type_id=w.id WHERE u.ext_id =?`;
    } else if (userData[0].roleName == ENTERPRISE) {
      queryReq = `SELECT u.ext_id as enterpriseId,u.first_name,u.last_name,u.email,u.phone,u.profile_pic,u.role,u.work_type_id,e.company_name,u.industry_type_id,u.is_active,e.siret_no,e.description,e.deliveryMonthHours,e.created_on,i.industry_type,i.industry_type_desc,c.country_name as country,s.state_name as state,ct.city_name as city,CASE WHEN u.is_active = 1 THEN 'Active' WHEN u.is_active = 2 THEN 'Rejected' WHEN u.is_active = 0 THEN 'Pending' ELSE 'Pending' END AS status FROM vw_rmt_user u LEFT JOIN rmt_enterprise e ON u.ext_id = e.ext_id LEFT JOIN rmt_industry_type i ON u.industry_type_id = i.id LEFT JOIN rmt_country c ON e.country_id=c.id LEFT JOIN rmt_state s ON e.state_id=s.id LEFT JOIN rmt_city ct ON e.city_id=ct.id WHERE u.ext_id =?`;
