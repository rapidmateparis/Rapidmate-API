...
exports.getJoinRequest = async (req, res) => {
  try {
    ...
    const countQuery = `SELECT COUNT(*) AS total FROM vw_rmt_user ${queryReq}`;
    const sql =
      `SELECT ext_id,first_name,last_name,email,phone,profile_pic,role,work_type_id,company_name,industry_type_id,is_active,CASE WHEN is_active = 1 THEN 'Active' WHEN is_active = 2 THEN 'Rejected' WHEN is_active = 0 THEN 'Pending' ELSE 'Pending' END AS status FROM vw_rmt_user ${queryReq}` +
  ...
  try {
    ...
    if (getId) {
      ...
      if (updatedItem.affectedRows > 0) {
        const updateUser=`UPDATE vw_rmt_user SET is_active=1 WHERE ext_id=?`
        const upstaus = await updateQuery(updateUser, [ext_id]);
        return res
          .status(200)
