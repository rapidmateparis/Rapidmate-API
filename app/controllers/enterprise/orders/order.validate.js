 ...
 */
exports.createItem = [
  check("enterprise_ext_id")
    .exists()
    .withMessage("MISSING")
    .not()
    .isEmpty()
    .withMessage("IS_EMPTY"),
  check("delivery_type_id")
    .exists()
    .withMessage("MISSING")
    .not()
    .isEmpty()
...
exports.updateItem = [
  ...
  check("repeat_mode")
    ...
    .exists()
    .withMessage("repeat mode field declare mandatory.")
    .not()
    .isEmpty()
    .withMessage("repeat mode field is required"),
  ...
  check("repeat_dropoff_location_id")
    .optional({ checkFalsy: true })
    .exists()
    .withMessage("repeat dropoff location field declare mandatory.")
    .not()
    .isEmpty()
    .withMessage("repeat dropoff location field is required"),
  check("id")
    .exists()
    .withMessage("MISSING")
    .not()
...
exports.orderItem = [
  ...
  body("delivery_type_id").custom((value, { req }) => {
    if (
      value === 2 &&
      (!req.body.addAnothers || req.body.addAnothers.length === 0)
    ) {
  ...
  check("addAnothers.*.to_latitude")
    .optional({ checkFalsy: true })
    .exists()
    .withMessage("latitude field declare mandatory.")
    .not()
    .isEmpty()
    .withMessage("to_latitude field is required"),
  check("addAnothers.*.to_longitude")
    .optional({ checkFalsy: true })
    .exists()
    .withMessage("to_longitude field declare mandatory.")
    .not()
    .isEmpty()
    .withMessage("to_longitude field is required"),
  check("addAnothers.*.delivery_date")
    .optional({ checkFalsy: true })
    .exists()
    .withMessage("delivery date field declare mandatory.")
    .not()
    .isEmpty()
    .withMessage("delivery_date field is required")
    .isISO8601()
    .withMessage("INVALID_DATE")
    .custom((value) => {
      const date = new Date(value);
  ...
  check("addAnothers.*.delivery_start_time")
    .optional({ checkFalsy: true })
    .exists()
    .withMessage("delivery date field declare mandatory.")
    .not()
    .isEmpty()
    .withMessage("delivery_start_time field is required")
    .matches(/^([01]\d|2[0-3]):([0-5]\d)$/)
    .withMessage("Invalid time format, expected HH:mm"),
  check("addAnothers.*.delivery_end_time")
    .optional({ checkFalsy: true })
    .exists()
    .withMessage("delivery date field declare mandatory.")
    .not()
    .isEmpty()
    .withMessage("delivery_end_time field is required")
    .matches(/^([01]\d|2[0-3]):([0-5]\d)$/)
    .withMessage("Invalid time format, expected HH:mm"),
  check("addAnothers.*.m_total_hours")
    .optional({ checkFalsy: true })
    .exists()
    .withMessage("delivery date field declare mandatory.")
    .not()
    .isEmpty()
    .withMessage("m_total_hours field is required")
    .isInt()
    .withMessage("Enter only digits value"),
  check("addAnothers.*.m_distance")
    .optional({ checkFalsy: true })
    .exists()
    .withMessage("delivery date field declare mandatory.")
    .not()
    .isEmpty()
    .withMessage("m_distance field is required")
    .isInt()
    .withMessage("Enter only digits value"),
  (req, res, next) => {
    validationResult(req, res, next);
