...
exports.getItem = async (req, res) => {
  try {
    ...
    const getUserQuerye = "select * from rmt_enterprise where ENTERPRISE_ID='"+id+"'"
    const data = await runQuery(getUserQuerye)
    let message="Items retrieved successfully";
    if(data.length <=0){
        message="No items found"
...
exports.createItem = async (req, res) => {
  try {
    const doesNameExists =await utils.nameExists(req.body.enterprise_name,'rmt_enterprise','ENTERPRISE_NAME')
    if (!doesNameExists) {
      ...
      return res.status(400).json(utils.buildErrorObject(400,'Enterprise name already exists',1001));
    }
  } catch (error) {
    return res.status(500).json(utils.buildErrorObject(500,'Something went wrong',1001));
  }
