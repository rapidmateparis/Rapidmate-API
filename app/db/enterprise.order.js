...
exports.INSERT_SHIFT_QUERY=`INSERT INTO rmt_enterprise_order_slot(enterprise_id,branch_id,delivery_type_id,service_type_id,vehicle_type_id,from_date,to_date,total_hours,is_same_slot_all_days) values((select id from rmt_enterprise where ext_id=?),?,?,?,?,?,?,?,?)`
...
exports.FETCH_SHIFT_BY_EXTID=`SELECT * FROM rmt_enterprise_order_slot WHERE is_del=0 AND enterprise_id=(select id from rmt_enterprise where ext_id=?)`
...
exports.UPDATE_SHIFT_QUERY = `UPDATE rmt_enterprise_order_slot SET enterprise_id =(select id from rmt_enterprise where ext_id=?),branch_id = ?,delivery_type_id = ?,service_type_id = ?,vehicle_type_id = ?,from_date = ?,to_date = ?,total_hours = ?,is_same_slot_all_days = ? WHERE id = ?;`;
...
exports.DELIVERY_BOY_ASSIGN_IN_SHIFT=`UPDATE rmt_enterprise_order_slot SET delivery_boy_id=(select id from rmt_delivery_boy where ext_id = ?) WHERE is_del=0 AND enterprise_id=(select id from rmt_enterprise where ext_id = ?) AND id=?`
